...
/** Parse comma-separated exit phrases (as displayed in the state editor). */
export function parseExitPhrases(text: string): string[] {
  return text.split(',').map(s => s.trim()).filter(Boolean);
}

/** Parse newline-separated exit phrases (as displayed in the arrow editor). */
export function parseExitPhrasesNewline(text: string): string[] {
  return text.split('\n').map(s => s.trim()).filter(Boolean);
}

/** Parse newline-separated keywords (as displayed in the arrow editor). */
