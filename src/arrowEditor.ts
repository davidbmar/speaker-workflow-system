/**
 * arrowEditor.ts — Click-to-edit panel for workflow arrows (transitions + keywords).
 */
...
export function openArrowEditor(
  ...
  if (ctx.isExitPhrase) {
    renderArrowEditorFields('Exit Phrases', [
      { key: 'exitPhrases', label: 'Exit Phrases (one per line)', value: state.currentDef.exitPhrases.join('\n'), multiline: true },
    ], ctx, state, dom, cb);
    return;
  }
  ...
  if (ctx.fromStateId && ctx.target) {
    ...
    if (isExit) {
      const exitMsg = ctx.target.startsWith('exit:') ? ctx.target.slice(5) : state.currentDef.exitMessage;
      fields.push({
        key: 'exitMessage',
